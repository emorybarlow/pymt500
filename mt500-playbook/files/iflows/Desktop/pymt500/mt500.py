...
class MT500:
    ...
    def send_heartbeat(self):
        ...
        if time.time() - self.last_hb > self.heartbeat_interval:
            ...
            try:
                requests.post('https://mtiv-tools.com/mapi/mt500_stats', data=json.dumps(stats), headers={'Content-Type': 'application/json'})
            except Exception as e:
    ...
    def send_to_consumers(self, record, raw):
        ...
        for consumer in self.consumers:
            ...
            if data_type == 'server':
                ...
                error = self.send_data(host, port, raw_bytes)
